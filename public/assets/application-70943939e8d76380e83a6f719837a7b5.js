...
function initialize_map() {
    ...
    var html = "<div id='infowindow-content'><table>" +
            ...
            "</select> </td></tr>" +
            "<tr><td></td><td><input type='button' value='Post' onclick='saveData()'/></td></tr></table></div>";
...
function placeDBMarkers(map) {
    for (var i = 0; i < gon.markers.length; i++) {
        if (gon.markers[i].latitude && gon.markers[i].longitude) {

            console.log(gon.markers[i]);

            var archiveMarker = new google.maps.Marker({
                map: map,
                position: {lat: gon.markers[i].latitude, lng: gon.markers[i].longitude}
            });
        }
    }
}
